...
export default function Index() {
  ...
  const handleOnDrop = (e: any) => {
    ...
    if (
      ...
      const hasProgressWidget = widgets?.some(
        (item) => item?.type === "progress"
      );
      const hasTimerWidget = widgets?.some((item) => item?.type === "timer");
      if (selectedWidget?.type === "progress" && hasProgressWidget) {
        alert("You can only add one progress bar");
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="absolute md:hidden flex w-[100%] justify-between px-3 bg-[#1d1160] py-5">
        <div className="flex gap-5">
          {SidebarConfig?.map((item) => {
            return (
              <div
                className="flex items-center border-[1px] px-4 py-3 border-[lightgray] bg-white rounded-md"
                draggable
                onDragStart={(e) => handleDrag(e, item)}
              >
                <h1 className="text-[12px] font-semibold">{item?.name}</h1>
              </div>
            );
          })}
        </div>
      ...
      <div className="w-[20%] hidden md:block fixed top-0 left-0 h-screen bg-white shadow-md">
      ...
      <div
        ...
        onDrop={handleOnDrop}
        className="bg-[white] w-full px-3 pt-[100px] md:px-40 md:mt-0 md:ml-[20%] "
      ...
      >
        ...
        {widgets?.length > 0 && (
          <div className="w-[full] mt-4">
            <button
              type="submit"
              className="w-full bg-[#1d1160] text-white py-2 rounded-md hover:bg-[#312286] transition text-[14px]"
